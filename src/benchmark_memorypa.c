...
void memorypa_initializer_options(memorypa_functions *functions, memorypa_pool_options *sets_of_pool_options) {
  ...
  sets_of_pool_options[1].power = 12;
  sets_of_pool_options[1].amount = 50;
  sets_of_pool_options[2].power = 13;
  sets_of_pool_options[2].amount = 60;
  sets_of_pool_options[3].power = 14;
  sets_of_pool_options[3].amount = 100;
  sets_of_pool_options[4].power = 15;
  sets_of_pool_options[4].amount = 200;
  sets_of_pool_options[5].power = 16;
  sets_of_pool_options[5].amount = 300;
  sets_of_pool_options[6].power = 17;
  sets_of_pool_options[6].amount = 560;
  sets_of_pool_options[7].power = 18;
  sets_of_pool_options[7].amount = 5;
}
    ...
    switch(operation) {
      case 0:
        if(blocks[index] == NULL) {
          ...
          blocks[index] = memorypa_malloc(current_block_size);
          ++malloc_count;
      ...
      case 1:
        if(blocks[index] == NULL) {
          current_block_size = (rand() * (index + 1)) % block_size_max + 1;
          blocks[index] = memorypa_calloc(current_block_size, 1);
          ++calloc_count;
          *((unsigned char *)blocks[index]) = 1;
          blocks_sizes[index] = current_block_size;
      ...
      case 2:
        if(blocks[index] != NULL) {
          current_block_size = (rand() * (index + 1)) % block_size_max + 1;
          realloc_result = memorypa_realloc(blocks[index], current_block_size);
          ++realloc_count;
